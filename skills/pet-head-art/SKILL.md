...
- Default output: one centered 1:1 head avatar on a clean white background, high resolution, no text/logo/watermark. Finish the complete pet/accessory silhouette as a sticker with a medium pure-white outline that directly touches the edge, plus one soft low-opacity neutral-gray shadow with a broad blur and slight downward offset. Keep the shadow subtle on white; never add a hard dark rim, glow, clear gap, double outline, or shadow inside the face. In head-only mode, isolate the natural head and headwear silhouette without leaving an exposed neck or partial torso. Change background color, aspect ratio, transparency, or the default sticker finish only when requested.
...
- For a wallpaper, create each approved subject as a tightly cropped transparent cutout with no baked outline, matte, gap, shadow, or background. The cutout may be head-only or intentionally body-inclusive. Let the compositor add the final white outline and soft shadow consistently after layout.
...
6. **Quality gate.** Reject any result with a pointed/narrow muzzle, changed eye or pupil proportions, shifted coat patches, extra animals/limbs, missing accessory-defining features, or text/logos. In head-only mode, reject exposed necks and partial torsos; in body-inclusive mode, require a coherent pose and complete intended anatomy. For a finished avatar, require one continuous medium white outline flush to the complete outer silhouette and one soft, light shadow; reject missing sections, transparent/color halos, double borders, hard dark shadows, internal facial outlines, or clipped effects. For wallpaper assets, reject opaque backgrounds, baked outlines/shadows, and surrounding whitespace. Regenerate only the failed item.
...
- For a finished avatar, apply the outline and shadow once around the combined pet/accessory silhouette, never around eyes, muzzle, individual fur patches, or accessory openings. Keep enough canvas margin that the blur is not clipped.
...
Use `--layout grid` only when the user requests an orderly grid. The compositor applies a consistent `8px` white outline and a soft low-opacity shadow by default, repeats inputs as needed for dense coverage, and never redraws the pet.
